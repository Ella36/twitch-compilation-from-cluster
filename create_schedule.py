...
class ReleaseSolver:
    ...
    def cost(self, candidate):
        avg_interval_day_cost = sum([abs(self.target_interval_per_day-d.cost) for d in candidate])
        spread_cost = 0
        for day_idx, d in enumerate(candidate):
            for r in d.releases:
                ...
                for day2_idx, d2 in enumerate(candidate):
                    n = [r2.name.split('_')[0] for r2 in d2.releases].count(release_name.split('_')[0])
                    if day2_idx == day_idx:
                        n -= 1
...
if __name__ == '__main__':
    ...
    while is_prompt_confirm('Generate a new schedule'):
        ...
        release_solver = ReleaseSolver(initial_release_by_day, target_interval_per_day)
